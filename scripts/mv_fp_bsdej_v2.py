...
"""MV-FP BSDEJ — FIXED version.

Bugs in v1:
  1. Scheduler never reset between outer iters -> LR near zero from iter 2+
  2. Early stopping triggers immediately when LR is zero
  3. Warmstart only for initial avg_comp=0.75 -> strong prior to wrong solution

Fixes:
  - Reset optimizer + scheduler state each outer iter (keeps network weights)
  - No early stopping in inner loop (always use full n_inner steps)
  - Warmstart ONLY before outer loop; trust FP to correct the prior

Run: python -u scripts/mv_fp_bsdej_v2.py
"""
...
def run_mv_fp(N=2, n_outer=10, n_inner=3000, damping=0.2, lr=5e-4):
    print(f"\n{'='*60}")
    print(f"MV-FP BSDEJ v2: N={N}, n_outer={n_outer}, n_inner={n_inner}, damping={damping}")
    ...
    for k in range(n_outer):
        ...
        with open(f"results_final/mv_fp_bsdej_v2_N{N}.json", "w") as f:
...
if __name__ == "__main__":
    all_results = {}
    # Only N=2 first — if it works, then do N=5, 10
    for N in [2]:
        try:
            gc.collect()
            avg, spread, err, hist = run_mv_fp(
                N=N, n_outer=8, n_inner=3000, damping=0.2
            )
            all_results[f"N={N}"] = {
    ...
    with open("results_final/mv_fp_bsdej_v2_all.json", "w") as f:
